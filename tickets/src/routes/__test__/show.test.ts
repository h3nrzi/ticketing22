...
it("returns 404 if the ticket is not found", async () => {
	...
	await request(app).get(`/api/tickets/${new mongoose.Types.ObjectId().toHexString()}`).expect(404);
});
...
it("returns the ticket if it is found", async () => {
	...
	const res2 = await request(app).get(`/api/tickets/${res.body.id}`).send().expect(200);
